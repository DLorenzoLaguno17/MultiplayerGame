		...
		if (state == ServerState::Listening)
		{
			...
			for (int i = 0; i < MAX_CLIENTS; ++i)
			{
				if (clientProxies[i].connected)
				{
					...
					ImGui::Text(" - Last packet time: %.04f", clientProxies[i].secondsSinceLastPacketSent);
	...
	if (state == ServerState::Listening)
	{
		...
		if (message == ClientMessage::Hello)
		{
			if (proxy == nullptr)
			{
				...
				else if (proxy->deliveryManager.processSequenceNumber(packet))
				{
					...
					for (uint16 i = 0; i < networkGameObjectsCount; ++i)
					{
						...
						// Replicate world state for new players
		...
		// Reset packet timer
		if (proxy != nullptr)
			proxy->secondsSinceLastPacketSent = 0.0f;
	}
}
...
void ModuleNetworkingServer::onUpdate()
{
	if (state == ServerState::Listening)
	{
		...
		secondsSinceLastPingToClients += Time.deltaTime;

		for (ClientProxy &clientProxy : clientProxies)
		{
			if (clientProxy.connected)
			{
				clientProxy.secondsSinceLastPacketSent += Time.deltaTime; 
				clientProxy.secondsSinceLastReplication += Time.deltaTime;

				// Disconnect clients whose time since the last received packet is greater than 5 seconds
				if (clientProxy.secondsSinceLastPacketSent > DISCONNECT_TIMEOUT_SECONDS)
				{
					OutputMemoryStream disconnect;
					disconnect << PROTOCOL_ID;
					disconnect << ServerMessage::Unwelcome;
					sendPacket(disconnect, clientProxy.address);
					destroyClientProxy(&clientProxy);
				}

				// Send a Ping to all clients every 0.5 seconds		
				if (secondsSinceLastPingToClients > PING_INTERVAL_SECONDS)
				{
					OutputMemoryStream pingPacket;
					pingPacket << PROTOCOL_ID;
					pingPacket << ServerMessage::Ping;

					if (clientProxy.deliveryManager.hasPendingAcks())
						clientProxy.deliveryManager.writePendingAcks(pingPacket);
...
void ModuleNetworkingServer::destroyNetworkObject(GameObject* gameObject)
{
	...
	for (int i = 0; i < MAX_CLIENTS; ++i)
	{
		...
		{
			// TODO(you): World state replication lab session
			clientProxies[i].replicationManager.destroy(gameObject->networkId, gameObject->destroyerTag);
		}
	}
